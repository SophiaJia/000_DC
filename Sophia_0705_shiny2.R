...
server <- function(input, output, session) {
  ...
  date_wrong <- eventReactive(input$date_check,{
    ...
    col_number <- ncol(di)-1
    show_col <- colnames(di)[3:col_number]

    for (i in 3: col_number){
      varname  <- paste0("col",i)
      di <-
        di %>% mutate(!!varname := ifelse(di[,i] > di[,i+1]|di[,i] < di[,i-1],1,0))
    }
    hind_col <- colnames(di)[(col_number+2):ncol(di)]
    tt <- di[hind_col] %>% rowSums(na.rm = T)
    date_check_data[['show']] <- show_col
    date_check_data[['hind']] <- hind_col
    di[tt>0,]
    #di
  })
  ...
  #                 backgroundColor = styleInterval(0, c('gray', 'white'))
  # ))
