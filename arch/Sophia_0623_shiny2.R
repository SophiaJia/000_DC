...
server <- function(input, output, session) {
  ...
  output$inpCategorical <- DT::renderDataTable({
    di <- outD()
    d_cat <- di[sapply(di, mode) == "character" | sapply(di, class) == "factor"]
  ...
  # output$datatmp <- DT::renderDataTable({
  #   d <- DataRow()
  #   
  #   # if hit tht eupdate butten then 
  #   input$goButton
  #   isolate(
  #     d[input$datatmp_rows_selected,3] <- "update"
  #   )
  #   d
  #   
  # })
  ...
  output$datatmp <- DT::renderDataTable(df())
  ...
  #output$y11 = renderPrint(input$datatmp_rows_selected)
  #output$y12 = renderPrint(input$cat_gt5_text)
  #output$y13 = renderPrint(input$action)

  #input$Catgt5_datatmp_rows_selected

  output$popup <- renderUI({
    bsModal("Catgt5",
            paste0("Variable Names: ",SelectedRowName()), "", size = "large",
            column(12,
                   DT::dataTableOutput("datatmp"),
