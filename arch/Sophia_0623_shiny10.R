...
server <- function(input, output, session) {
  ...
  DataRow <- eventReactive(input$select_button,{
   ...
   observeEvent(input$cat_pop_submit, {
     values_save[[SelectedRowName()]] <- values[[SelectedRowName()]]
     
   })
   
   observeEvent(input$refresh, {
     mydata <- isolate(inter_data[['afterfuzzy']])
     myvar  <-  values_save[[SelectedRowName()]]
     if(input$cat_pop_submit>0){
       for (i in 1:nrow(myvar)){
       mydata[mydata[,SelectedRowName()] == myvar[i,1] & !is.na(mydata[,SelectedRowName()]),SelectedRowName()] <- myvar[i,3]
