    ...
    mainPanel(
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Date",
                           ...
                           h4("Following is something wrong")#,
...
server <- function(input, output, session) {
  ...
  con_aftercheck_pre <- reactive({
    con_aftercheck_tmp()
    })

  output$con_aftercheck = renderDT(con_aftercheck_pre(), selection = 'none', rownames = FALSE, editable = TRUE)
  ...
  observeEvent(input$con_aftercheck_cell_edit, {
    ...
    v = info$value
    con_aftercheck_pre()[i, j] <<- DT::coerceValue(v, con_aftercheck_pre()[i, j])
    replaceData(proxy_con, con_aftercheck_pre(), resetPaging = FALSE, rownames = FALSE)
  })



  # https://yihui.shinyapps.io/DT-edit/   make it server side, it will able to edit and change.
